...
### Home Lab Hosts
- **nuc-1** (192.168.20.65): Intel NUC i7-1185G7 64GB, Iris Xe GPU - runs kometa + rclone-nzbdav
- **beelink** (192.168.20.66): Ubuntu mini PC i3-N305 - runs plex-previews + nzbdav WebDAV server + rclone-nzbdav
- **Synology** (192.168.20.70): NAS - Arr services + downloaders + rclone-nzbdav, NFS server for plex library
- **QNAP** (192.168.20.80): NAS - Plex Media Server (NVIDIA GTX 1660) + streaming services + rclone-nzbdav

### plex-previews Deployment (beelink)
- **Container**: `plex-previews` on beelink (Docker)
- **Image**: `plex-generate-previews:latest` (locally built on beelink)
- **Web UI**: http://192.168.20.66:8008 (reverse proxy: plexpreview.sakhter.org)
- **GPU**: Intel i3-N305 iGPU via VAAPI (`/dev/dri` device passthrough, render group GID 993)
- **Compose file**: `/opt/docker/plex-previews/docker-compose.yml` on beelink
- **DB (inside container)**: `/config/plex_previews.db`
- **DB (host path on beelink)**: `/opt/docker/plex-previews/config/plex_previews.db`

### Build & Deploy Workflow
Source code and deployment both live on **beelink**. Source at `/opt/projects/plex_generate_vid_previews` (also symlinked at `/home/ubuntu/projects/plex_generate_vid_previews`).

```bash
# 1. Build on beelink
cd /home/ubuntu/projects/plex_generate_vid_previews
docker compose build --no-cache

# 2. Restart on beelink
cd /opt/docker/plex-previews && docker compose up -d
```

### Checking Logs & DB on beelink
```bash
# Container logs (run locally on beelink, or via ssh)
docker logs plex-previews --tail 50

# Query the production database
docker exec plex-previews python3 -c "import sqlite3; conn=sqlite3.connect('/config/plex_previews.db'); c=conn.cursor(); c.execute('SELECT id,title,status,error_message FROM mediaitem WHERE id=ITEM_ID'); print(c.fetchone())"
```

**NOTE**: Since the code runs on beelink where we develop, the production database is at `/opt/docker/plex-previews/config/plex_previews.db` on beelink.

### Container Volume Mounts
The container mounts `/mnt:/mnt` from the host, giving it access to:
- `/mnt/plex` → NFS from Synology (`192.168.20.70:/volume2/plex-library`) - media library symlinks
- `/mnt/remote/nzbdav` → rclone-nzbdav container FUSE mount on beelink (50GB VFS cache, WebDAV from beelink:3003)
- `/mnt/remote/realdebrid` → NFS from QNAP (`192.168.20.80:/container-realdebrid`) - rclone-decyp FUSE mount
- `/mnt/plex-data` → NFS from QNAP (`192.168.20.80:/plex-config`) - Plex server config (BIF bundles written here)

### Path Flow for Media Files
Plex (on QNAP) sees media via symlinks in its library (`/mnt/plex`). The symlinks point to:
- `/mnt/nzbdav/.ids/{guid}` - nzbdav WebDAV content (Usenet streaming)
- `/mnt/remote/realdebrid/__all__/file.mkv` - Real-Debrid content

plex-previews resolves these same symlinks via its own `/mnt/remote/nzbdav` and `/mnt/remote/realdebrid` mounts on beelink.

### Startup Wait
Container uses `STARTUP_WAIT_PATH=/mnt/remote/nzbdav/content` to wait for the rclone-nzbdav mount to be ready before processing. The rclone-nzbdav compose on beelink includes a `restart-plex-previews` sidecar that restarts plex-previews when the FUSE mount recovers after disruption.

### Distributed rclone Architecture (2026-02-06)
Each host runs its own `rclone-nzbdav` container mounting WebDAV directly from beelink:3003. This eliminates the previous NFS+FUSE kernel deadlock issue. Post-start hooks restart dependent containers (plex-previews on beelink) when mounts recover.
